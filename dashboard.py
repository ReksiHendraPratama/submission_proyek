...
def load_data():
    ...
    # Mapping kategori musim agar lebih deskriptif (sesuai analisis Anda)
...
with tab1:
    ...
    sns.boxplot(data=day_df, x='season', y='cnt', palette='coolwarm', ax=ax1)
    ...
    sns.barplot(data=day_df, x='season', y='cnt', estimator=sum, palette='viridis', ax=ax2)
    ...
    ax2.set_ylim(0, day_df.groupby('season')['cnt'].sum().max() * 1.2)  # Tambah padding 20%
    ax2.set_yticks(range(0, int(day_df.groupby('season')['cnt'].sum().max()) + 100000, 100000))  # Set interval 100,000
    ax2.set_yticklabels([f'{x:,}'.format(x=x) for x in range(0, int(day_df.groupby('season')['cnt'].sum().max()) + 100000, 100000)])
    
    # Menambahkan label nilai di atas setiap bar
    for i, v in enumerate(day_df.groupby('season')['cnt'].sum()):
    ...
    season_stats = day_df.groupby('season')['cnt'].agg(['mean', 'median', 'min', 'max']).reset_index()
    st.dataframe(season_stats.style.format("{:.2f}", subset=['mean', 'median', 'min', 'max']))

    # Insight
    st.markdown("""
    **Insight:**
    - Musim panas (Summer) menunjukkan jumlah penyewaan tertinggi, baik dari distribusi (boxplot) maupun total penyewaan (barplot).
    - Musim dingin (Winter) memiliki jumlah penyewaan paling rendah, konsisten di kedua visualisasi.
...
with tab2:
    ...
    sns.lineplot(data=weekdays_df, x='hr', y='cnt', ci=None, color='blue', ax=ax2)
    ...
    sns.boxplot(data=weekdays_df, x='hr', y='cnt', palette='Blues', ax=ax3)
    ...
    st.markdown("""
    **Insight:**
    - Lonjakan penyewaan terjadi pada pukul 07:00-09:00 (pagi) dan 17:00-19:00 (sore).
    - Pola ini mencerminkan penggunaan sepeda untuk commuting (ke dan dari tempat kerja/sekolah).
    - Penyewaan cenderung rendah di siang hingga malam hari.
    """)
# Tab 3: Kesimpulan
with tab3:
    ...
    - **Musim panas** menunjukkan jumlah penyewaan tertinggi, kemungkinan karena cuaca yang hangat dan nyaman untuk bersepeda.
    - **Musim dingin** memiliki penyewaan terendah, dipengaruhi oleh suhu dingin dan kondisi cuaca yang kurang mendukung.
    - Implikasi: Strategi pemasaran dapat difokuskan pada musim panas untuk meningkatkan penyewaan.
    - Dapat disimpulkan bahwa musim mempengaruhi minat orang dalam bersepeda, saat musim dingin prang malas bersepeda karena di luar rumah keadaan snagat dingin. Saat musim panas yang hangat, orang lebih banyak bersepeda karena didukung oleh kondisi lingkungan yang baik

    ### 2. Pola Penyewaan Sepeda pada Hari Kerja
    - Terdapat **lonjakan signifikan** pada jam sibuk pagi (07:00-09:00) dan sore (17:00-19:00), menunjukkan penggunaan sepeda untuk keperluan komuter.
    ...
    - Dapat disimpulkan bahwa jam pergi sekolah/kerja dan jam pulang sekolah/kerja adaah jam dimana orang banyak menyewa sepeda untuk keperluan komuter
    """)
